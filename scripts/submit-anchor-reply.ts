...
try {
  const result = await createAnchorClaimRepository(client, uploadRuntime).submitReply({ caseId, sessionToken, body: await stdin(), evidence })
  process.stdout.write(`${JSON.stringify({ event: 'anchor_reply_submitted', ...result })}\n`)
} finally {
