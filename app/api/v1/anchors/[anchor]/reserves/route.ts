...
export async function GET(request: Request, context: AnchorReserveRouteContext) {
  ...
  if (!parsedAnchor.success) {
    return apiErrorResponse({
      ...
      status: 400,
      code: 'invalid_anchor',
      message: 'Anchor must be a valid canonical identifier',
      requestId,
      asOf: now,
    })
  }
  ...
  try {
    ...
    if (!model) {
      return apiErrorResponse({
        ...
        message: 'No verified anchor is available for this identifier',
        requestId,
        asOf: now,
      })
    }
    ...
    return apiJsonResponse(request, body, {
      ...
      etag: true,
      etagValue: { ...body, request_id: undefined },
    })
  } catch (error) {
    if (error instanceof InvalidAnchorReserveCursorError) {
      return apiErrorResponse({
        request, status: 400, code: 'invalid_pagination', message: error.message, requestId, asOf: now,
      })
    }
    console.error('Unable to load the public anchor reserve read model', {
      name: error instanceof Error ? error.name : 'Error',
    })
    return apiErrorResponse({
      request,
      status: 503,
      code: 'anchor_reserves_read_unavailable',
      message: 'The anchor reserve read model is temporarily unavailable',
      requestId,
      asOf: now,
    })
  }
